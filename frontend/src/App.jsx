...
function App() {
  ...
  useEffect(() => {
    ...
    async function loadTickerDetail() {
      ...
      try {
        ...
        // Don't update error state if request was aborted (expected behavior)
    ...
    // Cleanup: abort the request if selectedTicker changes or component unmounts
    return () => {
  ...
  useEffect(() => {
    // Clear results when user modifies the search query (only if it doesn't match the searched query)
    if (headlineSearchResults && headlineSearchQuery !== headlineSearchResults.query) {
      setHeadlineSearchResults(null);
    }
  }, [headlineSearchQuery]); // Only depend on headlineSearchQuery - the external trigger

  const handleTickerClick = (ticker) => {
    setSelectedTicker(ticker);
    // Scroll to ticker detail panel
    setTimeout(() => {
